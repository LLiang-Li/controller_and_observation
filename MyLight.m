...
function Outputs(block)
  if (block.InputPort(1).CurrentDimensions == 3)
      str = ['t = 0: .05:2*pi;' newline ...
      'x= sin(t)/2 + 0.5;' newline ...
      'y = cos(t)/2 + 0.5;' newline ...
      'patch(x, y,[' num2str(min(1, max(0, abs(block.InputPort(1).Data(1))))) ' ' num2str(max(0, min(1, abs(block.InputPort(1).Data(2))))) ' ' num2str(max(0, min(1,abs(block.InputPort(1).Data(3))))) '])'];
      set(gcbh, 'MaskDisplay', str);
      set(gcbh, 'MaskIconUnits', 'normalized');
  elseif(block.InputPort(1).CurrentDimensions == 1)
      color_set = int8(block.InputPort(1).Data(1));
      if (int8(0) == color_set)
          color_now = 'black';
      elseif(int8(1) == color_set)
          color_now = 'red';
      elseif(int8(2) == color_set)
          color_now = 'blue';
      elseif(int8(3) == color_set)
          color_now = 'green';
      else
          color_now = 'black';
      end
      str = ['color(''' color_now  ''')' newline ...
      'r = linspace(0,1, 10000);' newline ...
      't = linspace(0, 500*pi, 10000);' newline ...
      'x = r.*sin(t)/2 + 0.5;' newline ...
      'y = r.*cos(t)/2 + 0.5;' newline ...
      'plot(x, y)'];
      set(gcbh, 'MaskDisplay', str); 
      set(gcbh, 'MaskIconUnits', 'autoscale');
  else
